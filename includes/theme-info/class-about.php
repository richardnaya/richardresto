...
if ( ! class_exists( 'Richard_Resto_About' ) ) {
	...
	class Richard_Resto_About {
		...
		/**
		 * Singleton instance of Richard_Resto_About.
		 *
		 * @var Richard_Resto_About $instance Richard_Resto_About instance.
		 */
		private static $instance;

		/**
		 * Main Richard_Resto_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Richard_Resto_About ) ) {
				self::$instance = new Richard_Resto_About;
		...
		public function setup_config() {
			...
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://www.preciousthemes.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'richard-resto' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'richard-resto' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'richard-resto-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '1.0.0' );
				wp_enqueue_script( 'richard-resto-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '1.0.0' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'richard-resto-about', 'BusinessPointAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'richard-resto' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'richard-resto' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'richard-resto' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'richard-resto' );
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'Richard Resto','richard-resto' ) . '</th>';
		            echo '<th>' . esc_html__( 'Richard Resto Pro','richard-resto' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://www.preciousthemes.com/downloads/richard-resto-pro/" target="_blank" class="button button-primary button-hero">Richard Resto Pro</a></td>';
		            echo '</tr>';
