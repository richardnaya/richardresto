...
if ( ! function_exists( 'richard_resto_get_slider_details' ) ) :
	 ...
	 */
	function richard_resto_get_slider_details() {
		...
		if ( ! empty( $posts ) ) {
			...
			foreach ( $posts as $post ) {
				if ( has_post_thumbnail( $post->ID ) ) {
					...
					$output[ $p_count ]['excerpt']   = richard_resto_get_the_excerpt( absint($slider_excerpt_length), $post );
...
if ( ! function_exists( 'richard_resto_get_the_excerpt' ) ) :
	 ...
	 */
	function richard_resto_get_the_excerpt( $length = 0, $post_object = null ) {
		global $post;
...
if ( ! function_exists( 'richard_resto_fonts_url' ) ) {
	/**
	 * Register Google fonts.
	 *
	 * @return string Google fonts URL for the theme.
	 */
	function richard_resto_fonts_url() {
		...
		if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'richard-resto' ) ) {
			$fonts[] = 'Open Sans:300,300i,400,400i,600,600i,700,700i,800,800i';
		}

		if ( 'off' !== _x( 'on', 'Raleway font: on or off', 'richard-resto' ) ) {
