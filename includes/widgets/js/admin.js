...
jQuery(function($){
  ////////////////////////////////
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var BusinessKitImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        richard_resto_file_frame = wp.media.frames.richard_resto_file_frame = wp.media({
          button: {
            text: jQuery( this ).data( 'uploader_button_text' )
          },
          state : 'rtam-insert-image',
              states : [
                  new BusinessKitImage()
              ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

        // When an image is selected, run a callback.
        richard_resto_file_frame.on( 'select', function() {

          var state = richard_resto_file_frame.state('rtam-insert-image');
